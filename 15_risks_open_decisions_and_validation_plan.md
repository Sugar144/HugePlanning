...
| RSK-A14 (V2) | AI-generated methodology files look right but behave wrong | False confidence, defects surface late | Every stage gate requires behavioural validation, not file review (`13` §2); scenario-first acceptance; "generated ≠ correct" stated as stage principle; stage assurance sequence (`22`) | Medium |
...
| RSK-A16 (S0a lesson) | Verification-oracle defects: a failing check misread as a product failure, or an inconclusive environment converted into a pass/fail (S0a: SPK sentinel false negative) | Working behaviour "fixed" into breakage; wasted correction cycles; false confidence | Mandatory failure classification before any fix (`22` §5); runtime results distinguish fake-runtime / live-runtime / inconclusive / genuine failure; oracle corrections are `TEST`-class changes | Low–Medium |
...
5. **Review cadence:** after S4 and after S8, re-read files `04`/`05`/`09` against observed behaviour and patch via methodology releases — the plan is versioned with the methodology from S0a onward (realized: this repository *is* the methodology repository and carries the planning corpus with it; no separate copy).
6. **Stage assurance (S0b onward):** every implementation stage and post-release maintenance change runs the proportional assurance sequence of `22` — contract reconciliation through release evidence, at the depth its `22` §6 factors warrant. The audited S0a integration is the reference execution.
