...
|---|---|---|---|---|---|---|---|---|---|
| 1 | `client-discovery` | strongest | Stage `discovery`, G0 passed | project.yaml, lock, client-materials, question-bank & knowledge | evidence/interviews/**, requirements/context/OQ drafts | Read, Write, Edit (repo-scoped) | Full contract `04` §1 | DoD `04` §12 + your acceptance | paused, aborted, blocked-on-client |
...
| 4 | `doc-generator` | strong | G1 passed; artifacts change post-approval | layer-2 artifacts | PRD, validation-package, product-backlog, status report | Read, Write | Introduce un-ID'd statements; edit layer 2 | docs generated + `generated_from` stamped | source-invalid → abort with report |
...
4. **interview-evidence-capture** (1) — every turn → dialogue → transcript.md/.jsonl appends, anchors, state persistence per `04` §6 contract → check: no candidate without source_ref.
...
12. **backlog-refinement** (2) — post-SDD; post-CR → approved REQs + SDD → delivery-backlog.yaml → decomposition rules `08` §1, DoR fields complete.
...
|---|---|---|
| evidence-policy | always | Evidence append-only; every fact needs source_ref; PII minimization; corrections are new turns |
...
## 5. Knowledge files (with "consult when")
...
Onboarding: you+scripts · Discovery: agent 1 · Analysis/spec: agents 3,4 + skills 5–9 · Technical design: agent 2 + skills 11,15 · Backlog/Jira: skills 12,16 · Implementation: agents 5–8 + skills 13,14 · Testing: skill 15 + `10` · Git/CI/deploy: `11` + agent 9 · Client validation: you + skill 9 outputs · Operations/change: `12` + agent 9 · Methodology evolution: you + `02` §10. No orphan stages; no agent without artifacts; no artifact without a consumer (`06` §1).
