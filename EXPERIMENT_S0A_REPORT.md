...
Tests re-run after corrections: 87/87 green.

## 12. Post-commit verification (executed)

1. `check-methodology-clean.sh` → CLEAN at the primary commit.
2. **Live launch on the real methodology path:** `start-agent.sh "<scratchpad>/spk client A" client-discovery -- -p "Follow your agent instructions now."` → **PASS**: preconditions ran, lock/checkout mismatch warning emitted as designed (lock pins the pre-S0a commit), agent resolved from the real `--add-dir` path, stub printed `SPK01-AGENT-OK` and the correct project state, post-session drift check reported "Methodology unchanged", exit 0.
3. **Defect found in the primary commit:** `scripts/lib/__pycache__/*.pyc` bytecode was committed — generated by the test suite's `python3 -m py_compile` step, which would also dirty the methodology tree on every test run and trip `check-methodology-clean.sh`. **Fix commit applied:** bytecode removed from git, `__pycache__/`/`*.pyc` added to the methodology `.gitignore` (pre-existing entries preserved), and the test's syntax check switched to `ast.parse` (writes nothing). `tests/run-tests.sh` re-run: **87/87 green** and the tree stays clean after a test run (verified via `git status`).

**Correction cycles used: 2** — one bounded pre-commit pass (§11), one post-commit fix commit (§12.3); within the ≤2 bound (DEC-20).
