...
5. **Review cadence:** after S4 and after S8, re-read files `04`/`05`/`09` against observed behaviour and patch via methodology releases — the plan is versioned with the methodology from S0a onward (realized: this repository *is* the methodology repository and carries the planning corpus with it under `planning/`; no separate copy).
