...
### R2-36 — Historical Claude.ai prototype baseline preserved (2026-07-12)

- **Validated?** Yes. Seven pre-V2 prototype skills (`business-discovery-interviewer`, `technical-ux-interviewer`, `artifact-pack-generator`, `documentation-quality-reviewer`, `mvp-scope-reducer`, `change-delta-interviewer`, `client-meeting-pack-preparer`) plus the Claude.ai project instructions were recovered from `~/Documents/HugePlanning-import/`. They predate V2, were never behaviorally validated under the Claude Code runtime, and contain genuinely reusable interview craft (pacing, fatigue handling, parking-lot discipline, scope-reduction content) alongside mechanisms V2 supersedes (document-at-close outputs, label taxonomy, artifact packs).
- **Decision:** **ACCEPT — preserve verbatim, mine as source material, never activate directly.** New directory `planning/history/claude-ai-prototypes/`: original `.skill` archives byte-identical (SHA-256 recorded), extracted copies for reading, project instructions, and a README carrying the mandatory labels (*historical prototype — not active runtime — not behaviorally validated*), the full reuse matrix (REUSE/ADAPT/HARDEN/SUPERSEDE/DISCARD/MISSING), and the prototype→V2 label mapping. Any revived behavior must be re-authored against the `04`/`05`/`14` contracts — the archives are never unpacked into `.claude/skills/`.
- **Reasoning:** the prototypes are evidence of prior method-design work and the primary conversational-craft input to S1; preserving them outside the runtime keeps invariant boundaries intact (nothing unvalidated becomes loadable), while the recorded assessment prevents both blind copying and from-scratch redesign.
- **Affected files:** new `planning/history/claude-ai-prototypes/**`; root `README.md` repository map row; `planning/README.md` tree. No runtime path touched.
- **Validation:** documentation-only per `22` §6; deterministic suite re-run as regression evidence; SHA-256 comparison against the recovered originals (8/8 identical).

### R2-37 — Product-spec foundation: the methodology's own requirements and backlog (2026-07-12)

- **Validated?** Yes. From S0b onward the repository's own implementation work needs a versioned contract (requirements, backlog, task packets, verification pointers) so stages are built against reviewable acceptance criteria instead of ad-hoc session prompts — without duplicating the roadmap (`13`) or inventing parallel conventions (V2 risk #4).
- **Decision:** **ACCEPT — minimal `product/` directory, own tiny schemas, suite-validated.** `product/requirements.yaml` + `product/backlog.yaml` (+ `product/tasks/TASK-nnn.md` packets) reuse the conventions rule verbatim (ID grammar, status enums, `schema_version`); `source_refs` cite plan sections, preserved prototypes, reports, and decisions; verification evidence is typed pointers only (`22` §7). Validated by two **methodology-internal** schemas, `product-requirements.schema.json` 1.0.0 and `product-backlog.schema.json` 1.0.0, plus a link-integrity check, all in the deterministic suite (new block T14). **Explicitly rejected:** reusing the client `requirements.schema.json` for product requirements — its semantics are deliberately client-bound (`origin` provenance enum, evidence-anchor `source_refs`, `proposed_default`, gate-bound `approved_in`) and product items cannot honestly populate them. **Boundaries:** product schemas are never listed in `methodology.lock.yaml`, never checked by `validate.sh`, never replicated to clients; scope is hard-limited to in-flight stages (today S0b+S1, schema-enforced `stage` enum).
...
### R2-39 — Defect-classification taxonomy reconciled and extended to seven classes (2026-07-12)

- **Validated?** Yes — a real contract discrepancy surfaced before S1 scenario defects could be recorded. The versioned normative set (`22` §5, from R2-34) is `CODE | TEST | CONTRACT | PROCESS | ENVIRONMENT`; the operator-level project summary used `CODE | TEST | SPEC | DATA | OPERATOR | ENVIRONMENT`. Not semantically identical: `SPEC` ≈ `CONTRACT` (naming only), but `DATA` and `OPERATOR` name failure modes the versioned set cannot express — and S1 scenario execution makes both material for the first time (import material can itself be defective; live-assisted relay and golden scoring are human procedures that can be wrong). `PROCESS` in turn has no equivalent in the operator summary. Diagnosis: partly operator-summary staleness (`SPEC`), partly a genuine missing failure-class distinction in `22` §5 (`DATA`, `OPERATOR`).
- **Decision:** **ACCEPT — seven classes**, extending the normative table in `22` §5 (compatible addition, no existing class removed or renamed): `CODE` implementation defect · `TEST` test/oracle/fixture/scoring-instrument defect · `CONTRACT` requirement/specification/methodology-contract defect · `DATA` malformed, incomplete, contaminated, or unsuitable scenario/input data · `OPERATOR` incorrect execution, relay, scoring, or human-procedure error in a run · `PROCESS` workflow, sequencing, or prescribed-process defect · `ENVIRONMENT` runtime/tooling/limits/platform defect. **`SPEC` is normalized to `CONTRACT`** — a mapping, not a competing synonym. `OPERATOR` (a human erred *executing* a sound procedure) is distinct from `PROCESS` (the prescribed procedure itself allowed the defect); conflating them mis-targets fixes.
- **Affected files:** `22` §5 (table + mapping note), `13` §2 (inline list), `product/requirements.yaml` FR-019-AC-04, `product/tasks/TASK-015.md`, `tests/golden-artifacts/01-clear/golden-checklist.md`, Draft PR #2 body. **Historical records keep their original five-class wording** (R2-34 entry, S0a records, README SPK log): pre-R2-39 classifications map unchanged into the new set (no old class was renamed); `SPEC`, where it appears in operator-side notes, reads as `CONTRACT`.
- **Validation:** documentation/contract reconciliation per `22` §6 (no runtime surface); deterministic suite re-run green; discrepancy recorded here rather than silently resolved (invariant 2).
