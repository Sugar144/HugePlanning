...
These rules apply repository-wide. More specific `AGENTS.md` files may add stricter scoped requirements.

- Inspect every applicable instruction file before modifying scoped content.
- Treat committed artifacts and explicit status records as durable truth; conversation context is not a substitute.
- Preview material changes by stating their paths, effects, validation, and authorization boundary.
- Never infer authorization. Modification, commit, push, pull request, merge, tag, release, deployment, and publication are separate permissions.
- Route deterministic parsing, hashing, comparison, serialization, counting, packaging, and validation to scripts first.
...
The canonical operating semantics are in `governance/methodology/project-operating-contract.md`.
