...
- **Decision:** **ACCEPT.** Three profiles `LITE / STANDARD / HIGH-RISK` (names kept; a 4th level rejected — no evidence it pays for a solo operator). Eight archetypes (revised from V1's five). New file `21_process_profiles_and_archetypes.md` holds the full model; subsystem files hold their own floors.
...
- **Decision:** **ACCEPT** — realized principally *through* R2-01: LITE fast path (combined G1+G2 single validation email; combined G6+G7+G8 single release approval; no Jira; artifact floor of 6 files; walkthrough in `21` §7 shows a landing page carrying ≈ 6–10 hours of total process overhead). Plus: maintenance expectations become named billable tiers in `engagement.md` (`12`); client-facing touchpoints remain plain-language email/call — the client never touches Git/Jira/Claude (already V1 policy, now stated as an explicit invariant); estimates price phases from the product backlog.
