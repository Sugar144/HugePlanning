...
- `GOV-AUD-001-P02-R1` — PASS-02 source execution under `GOV-AUD-AUTH-002` and exact amended prompt `GOV-AUD-PROMPT-021` / `HP-PROMPT-029`; its immutable outputs were independently reviewed in `GOV-AUD-001-P02-IER-002` with result `PASS_02_R1_CONFIRMED`. The Project Owner accepted PASS-02 and approved CHECKPOINT-A in `GOV-AUD-DECISION-003/0.1.0`. PASS-03 is authorized for preparation only; no PASS-03 contract, prompt, or execution exists.
