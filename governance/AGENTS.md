...
- Classify an explicit Project Owner authorization as `OWNER_PUBLICATION_AUTHORIZATION`, not a material prompt, only when it identifies an already reviewed immutable candidate, adds no implementation scope, authorizes only named atomic stage/commit/push actions, and excludes PR, merge, release, execution, ratification, risk acceptance, and further modification. Preserve it without requiring a pre-publication repository mutation.
