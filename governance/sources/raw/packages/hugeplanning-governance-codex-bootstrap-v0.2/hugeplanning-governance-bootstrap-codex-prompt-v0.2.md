# HugePlanning GOV-0 Repository and History Bootstrap — Codex Execution Contract v0.2
...
## 1A. CODEX-NATIVE INSTRUCTION CHAIN

This execution uses Codex's native repository-instruction mechanism.

Before this Codex session starts, the operator should place this temporary file at the repository root:

```text
AGENTS.override.md
```

It contains task-specific bootstrap constraints. Codex loads instruction files once at the start of a run/session, so:

- treat the loaded root `AGENTS.override.md` as authoritative for this GOV-0 run;
- do not modify it during the task except to remove it at the very end;
- do not assume a newly created instruction file affects the current session;
- create `governance/AGENTS.md` for future governance sessions;
- remove the temporary root `AGENTS.override.md` before the final scope check and commit;
- never commit the temporary root override.

At the beginning, report the active instruction sources you can identify and verify that the workspace root is the intended HugePlanning worktree.

The persistent `governance/AGENTS.md` created during this run must apply only to work launched from `governance/` or its descendants. It must not pretend to govern Claude Code, the runtime, or unrelated repository work.
...
The worktree may contain untracked files only in:
...
`AGENTS.override.md` is a temporary, task-specific Codex instruction file. It must remain untracked and be removed before the final validation.
...
Create concise persistent instructions for future Codex sessions launched from `governance/` or below.

It must include:

- scope: governance design, review, provenance, adoption planning, and later governance implementation only;
- read order: `README.md`, `CURRENT_STATE.md`, `GOVERNANCE_MASTER_PLAN.md`, applicable run manifest, then task-specific inputs;
- authority warning: proposed artifacts are not ratified or operational;
- preservation rule: raw sources are immutable and checksum-protected;
- provenance rule: prompts, inputs, outputs, decisions, limitations, and run status must be registered;
- update rule: any completed governance run must update its run manifest, artifact registry, current state, and relevant plan status;
- runtime boundary: no runtime changes without an explicit integration/adoption task and authorized paths;
- truthfulness: never fabricate retrospective evidence, execution, independence, validation, acceptance, or ratification;
- stop conditions: authority conflict, provenance conflict, missing core package, forbidden-path change, destructive ambiguity;
- validation expectations for Markdown, YAML, links, checksums, and changed paths;
- communication: operator interaction in Spanish; repository artifacts in English.

Keep it short enough to remain useful. Do not copy the full Kernel, master plan, or task prompt into it.
...
13. verify the temporary root `AGENTS.override.md` has been removed and is not staged or committed;
14. verify `governance/AGENTS.md` exists and is concise, scoped, and consistent with this execution contract.
...
Before this check, remove the temporary untracked root file:
...
1. Verify repository, branch, HEAD, worktree, active Codex instruction sources, and input files.
2. Confirm that the temporary root `AGENTS.override.md` was loaded for this session.
3. Read the existing repository orientation documents.
4. Inventory and checksum the inbox.
5. Present a compact import plan based on the actual files.
6. Continue with organization without asking permission unless a true blocking provenance decision exists.
